...
//	int s = 'r';
//	String stg = String.valueOf((char) s);
//	System.out.println(stg);
	
	for (String str : groupList) {
	    List<Integer> foundChars = new ArrayList();
	    String[] answersPerPerson = str.split("\r\n");

	    for (String ans : answersPerPerson) {
	    ...
	    outer: for (int i = 0; i < foundChars.size(); ++i) {
		int foundChr = foundChars.get(i);
		for (String ans : answersPerPerson) {
		    String chr = String.valueOf((char) foundChr);
		    if (!ans.contains(chr)) {
			foundChars.remove(i);
			// decrease i because we just removed an element
			--i;
			continue outer;
		    }
